...
import { restoreAccountFromSSO } from './common/utils/sso.utils';
import { getErrorMessage } from './common/utils/string.utils';
import { SnackbarType, useSnackbar } from './hooks/use-snackbar.hook';
import useVersionCheck from './hooks/use-version-check.hook';
import { SidebarProvider } from './pages/dashboard/parts/sidebar/sidebar.provider';
...
export default function App() {
  ...
  // Restore account data from SSO token on app load
  useEffect(() => {
    restoreAccountFromSSO();
  }, []);

  const theme = createTheme({
    ...MODE[system?.mode || Mode.LIGHT],
    ...MODE.OTHER,
    components: {
      MuiStack: {
        styleOverrides: {
      ...
      MuiButton: {
        ...
        styleOverrides: {
          root: {
            '&.Mui-disabled': {
              borderColor: MODE[system?.mode || Mode.LIGHT].palette.divider,
            },
            fontWeight: 400,
            lineHeight: 'unset',
      ...
      MuiTooltip: {
        styleOverrides: {
          tooltip: {
            backgroundColor: MODE[system?.mode || Mode.LIGHT].palette.background.paper,
            color: MODE[system?.mode || Mode.LIGHT].palette.text.primary,
            boxShadow: MODE[system?.mode || Mode.LIGHT].shadows[1],
      ...
      MuiDivider: {
        styleOverrides: {
          root: {
            borderColor: `${MODE[system?.mode || Mode.LIGHT].palette.divider}${OPACITY[30]}`,
          },
        },
      },
      MuiTableCell: {
  ...
  useEffect(() => {
    (async () => {
      try {
        if (account.user) await dispatch(ACTION_ACCOUNT.getAccount(account.user.id)).unwrap();
      } catch (error) {
    ...
    (async () => {
      try {
        ...
        if (permission === 'granted' && account.isLogin && !account.fcmToken) {
          ...
          if (fcmToken) await dispatch(ACTION_ACCOUNT.subscribeTopic({ fcmToken })).unwrap();
        }
      } catch (error) {
        showSnackbar({ message: getErrorMessage(error), type: SnackbarType.ERROR });
      }
  ...
  useEffect(() => {
    const handler = async (event: MessageEvent) => {
      ...
      await dispatch(ACTION_ACCOUNT.changeNotificationCount(currentCount + 1)).unwrap();

      showSnackbar({ message: `${title}: ${message}`, type: SnackbarType.INFO });
    };
