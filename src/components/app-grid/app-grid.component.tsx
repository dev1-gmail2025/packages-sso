...
import { generateSSOTokenForTransfer } from '../../common/utils/storage.utils';
...
export const AppGrid: React.FC<AppGridProps> = ({
  ...
  // Handler cho click app
  const handleAppClick = (app: AppModule) => {
    if (app.url) {
      if (openUrlInNewTab) {
        ...
        if (ssoToken) {
          // Open through SSO bridge on target domain
          // Example: https://target.domain/sso-bridge.html?sso_token=...&redirect=<final app url>
          const target = new URL(app.url);
          const bridgeUrl = new URL('/sso-bridge.html', target.origin);
          bridgeUrl.searchParams.set('sso_token', ssoToken);
          bridgeUrl.searchParams.set('redirect', app.url);
          const finalUrl = bridgeUrl.toString();
    ...
    // Gọi onClickItem callback
    onClickItem?.(app);
  };

  return (
    <Box sx={{ position: 'relative' }}>
      <Box sx={{ display: 'grid', gridTemplateColumns: `repeat(${columns}, 1fr)`, gap }}>
        {visibleApps.map((app, index) => {
          ...
          return (
            ...
            >
              ...
              <Typography variant={captionVariant} sx={{ color: captionColor ?? theme.palette.common.white }}>
                {app.caption}
              </Typography>
            </MotionBox>
          );
