...
import { BORDER_RADIUS_ELEMENT_MIDDLE, GAP_ICON_CONTENT_BY_SIZE } from '../../../common/constant/style.constant';
...
export const TextFieldElement: React.FC<TextFieldElementProps> = ({
  ...
  return (
    <Box sx={{ position: 'relative', width: '100%' }}>
      <TextFieldLabelElement label={label} iconLabel={iconLabel} colorLabel={colorLabel} required={rest.required} />
      <TextField
        error={Boolean(error)}
        ...
        InputProps={{
          ...
          sx: {
            ...InputProps?.sx,
            // padding: `${STYLE.PADDING_GAP_ITEM_SMALL} ${STYLE.PADDING_GAP_ITEM_SMALL}`,
          },
        }}
      ...
      {description && (
        ...
        >
          <Box sx={{ fontWeight: typography.h2.fontWeight }}>Ghi chú:</Box> {description}
        </Typography>
      )}
    </Box>
  );
