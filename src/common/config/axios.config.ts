...
const createAxiosRequest = (baseURL: string): AxiosInstance => {
  ...
  axiosInstance.interceptors.response.use(
    ...
    async (error) => {
      ...
      if (errorData.statusCode === 777) {
        ...
        refreshingToken = refreshingToken || store.dispatch(ACTION_ACCOUNT.refreshToken({ refreshToken })).unwrap();
...
export const axiosRequest = createAxiosRequest(process.env.REACT_APP_BE_URL!);
export const axiosRequestOffice = createAxiosRequest(process.env.REACT_APP_BE_OFFICE_URL!);
