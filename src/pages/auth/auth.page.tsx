import { Grow, InputAdornment, Stack, useTheme } from '@mui/material';
import { Form, Formik, FormikHelpers } from 'formik';
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { authApi } from '../../apis';
import { ForgotPasswordType, ResetPasswordType } from '../../apis/auth/auth.enum';
...
export const AuthPage: React.FC<AuthPageProps> = ({}) => {
  ...
  const onSubmit = async (authDto: AuthDto, { setFieldValue, resetForm }: FormikHelpers<AuthDto>) => {
    setLoading(true);
    try {
      switch (authProcess) {
  ...
  return (
    <Grow in={true} timeout={1000}>
      <Stack sx={{ flex: 1, position: 'relative' }}>
        <Formik
          initialValues={{ password: '', token: '', otp: '', phone: '' }}
            ...
            return (
              <Form noValidate>
                ...
                >
                  ...
                  {(authProcess === AuthProcess.LOGIN || authProcess === AuthProcess.FORGOT_PASSWORD) && (
                    <TextFieldElement
                      iconLabel="mail"
                      label="Số điện thoại"
                      name="phone"
                  ...
                  {authProcess === AuthProcess.VERIFY_OTP && <OtpPart setAuthProcess={setAuthProcess} />}
