...
Thư mục `common` chứa tất cả các **hằng số, enum, interface và utility function** dùng chung toàn bộ dự án `sevago-sso-fe`. Mọi logic/type nào được tái sử dụng ở nhiều nơi đều nên đặt tại đây.
...
const blacklist = getAppBlackList(
  currentUser,
  getCurrentEnvironment(),
  {
